...
import { GREEN, PURPLE, SURFACE } from './palette';
...
export class Minimap {
  ...
  draw(
    ...
    ctx.fillStyle = 'rgba(232, 230, 225, 0.16)';
    ...
    ctx.clip();
    ctx.fillStyle = 'rgba(176, 77, 240, 0.16)';
    ctx.fill(this.profile!);
    ctx.restore();
    ...
    if (markers) {
      for (const marker of markers) {
        ...
        ctx.fillStyle = marker.isLeader ? PURPLE : marker.isElite ? GREEN : 'rgba(232, 230, 225, 0.55)';
    ...
    ctx.strokeStyle = 'rgba(232, 230, 225, 0.6)';
    ctx.lineWidth = 1;
    ctx.setLineDash([3, 3]);
