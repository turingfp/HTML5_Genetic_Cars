...
function lineageStyle(lineage: number): CarStyle {
  return {
    body: lineageColour(lineage, 70, 58, 0.35),
    stroke: lineageColour(lineage, 78, 66),
    spoke: lineageColour(lineage, 70, 62, 0.28),
    wheel: 'rgba(232, 230, 225, 0.1)',
    wheelStroke: lineageColour(lineage, 78, 66),
    alpha: 1,
  };
}
...
export class Renderer {
  ...
  private drawSky(): void {
    ...
    gradient.addColorStop(0.55, '#121216');
  ...
  private drawTerrain(track: TrackDef, minX: number, maxX: number): void {
    ...
    // The ground is solid asphalt, clearly denser than the night sky, so the
    // horizon always reads.
    ...
    // The painted edge line of the circuit: white, like the track limits it
    // is. It keeps the terrain profile readable at any zoom.
